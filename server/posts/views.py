    ...
    def vote(self, request, pk=None):
        ...
        if serializer.is_valid():
            try:
                ...
                if total_votes > 0:
                    post.honesty_score = int((post.upvotes / total_votes) * 100)
                else:
                    post.honesty_score = 50  # Default when no votes
                ...
                if not vote_removed:
                    # Vote wasn't removed, so it exists
                    user_vote = 1 if is_upvote else -1
